...
window.isset = function (v) {
  if ((typeof v === 'undefined' ? 'undefined' : _typeof(v)) == 'object' && v == 'undefined') {
    return false;
...
(function ($) {
  ...
  var utils = {
    ...
    isEventOut: function isEventOut(blocks, e) {
      var r = true;
  ...
  var modal = {
    ...
    getParentEl: function getParentEl(el) {
      var r = $(el);
    ...
    // Переход
    transition: function transition(el, action, options, callback) {
    ...
    prepare_body: function prepare_body(D, $this) {
    ...
    init_el: function init_el($this, options) {
      ...
      if (D.closeOnOverlayClick) D.overlay.block.add(D.container.block).click(function (e) {
        if (utils.isEventOut($('>*', D.body), e)) $this.arcticmodal('close');
      });
  ...
  var actions = {
    ...
    show: function show() {
      ...
      if (D.wrap.css('overflow') != 'hidden') {
        ...
        if (w2 != w1) D.wrap.css('marginRight', w2 - w1 + 'px');
      }
      ...
      if ($.isFunction(this)) {
        ...
        return this.each(function () {
          ...
          modal.transition(D.container.block, 'hide', modals.length > 1 ? { type: 'none' } : D.closeEffect, function () {
            ...
            if (!D.clone) $('#arcticmodalReserve' + D.modalID).replaceWith(D.body.find('>*'));

            D.overlay.block.remove();
            D.container.block.remove();
            $this.data('arcticmodal', null);
            if (!$('.arcticmodal-container').length) {
              if (D.wrap.data('arcticmodalOverflow')) D.wrap.css('overflow', D.wrap.data('arcticmodalOverflow'));
  ...
  $(document).bind('keyup.arcticmodal', function (e) {
    ...
    if (D.closeOnEsc && e.keyCode === 27) m.arcticmodal('close');
  });
...
(function ($) {
  ...
  $.jGrowl = function (m, o) {
    if ($('#jGrowl').size() == 0) $('').appendTo('body');
    $('#jGrowl').jGrowl(m, o);
  };
...
document.addEventListener("DOMContentLoaded", function (event) {
  var buttonToogle = document.querySelector('.page__header .toogle-btn'),
  ...
  //попап
  for (var i = 0; i < serviseBtn.length; i++) {
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  sc.smoothopaque = function (el, todo, speed, endFunc) {
    ...
    function slowopacity() {
      if (startop < endop) {
        op += 5;
        if (op < endop) window['top'].timeout = setTimeout(slowopacity, speed);else endFunc && endFunc();
      } else {
        op -= 5;
        if (op > endop) {
          window['top'].timeout = setTimeout(slowopacity, speed);
        } else endFunc && endFunc();
      }
      // установка opacity
